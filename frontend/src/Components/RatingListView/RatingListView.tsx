...
const RatingListView = ({ ratingApiUrl, onUserPage }: RatingListViewProps) => {
  const [ratings, setRatings] = useState<RatingDetailType[]>([]);
  const ratingListViewClassName = onUserPage
    ? "ratingViewUserPage"
    : "ratingListView";
      ...
      .then((response) =>
        response.json().then((data) => {
          ...
          if (onUserPage) {
            ...
            Promise.all(
              data.map((rating: RatingDetailType) =>
                fetch(
                  `http://localhost:8080/api/gamecard/get/id/${rating.gameCardId}`
                )
    ...
    <div className={ratingListViewClassName}>
      {ratings.map((rating) => (
        <div className="profilePageDiv" key={rating.gameCardId}>
          <h2>{rating.title}</h2>
          <RatingDetail gameId={+rating.gameCardId} rating={rating} onUserPage={onUserPage} gameCardId={rating.gameCardId}></RatingDetail>
        </div>
      ))}
      {ratings.length === 0 && (
        <p>
